(function (angular) {
    ...
    app.controller('movie_listController', [
        ...
        '$location',
        'MyService',
        function ($scope, $http, $routeParams, $route, $location, MyService) {
            $scope.loading = true // 为true显示加载动画
            // animate(obj, "width", pWidth*0.5);
            ...
            MyService.jsonp('http://api.douban.com/v2/movie/' + $routeParams.movieType,
                {start: start, count: $scope.pageSize, q: $routeParams.q}, function (data) {
                    ...
                    //顶部加载
                    animate(obj, { "width": pWidth},50, function () {
...
// 顶部加载动画
            ...
            var pWidth = progress.clientWidth;
            console.log(pWidth);

            $scope.$watch('loca.url()', function() {
                animate(obj, { "width": 780},30, function () {
